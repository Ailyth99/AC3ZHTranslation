...
public class UserInterface extends javax.swing.JFrame {
    ...
    public UserInterface() {
        initComponents();
        //this.setIconImage(new ImageIcon(getClass().getClassLoader().getResource("Neucom.png")).getImage());
        ...
        try {
            ...
            InputStream is = UserInterface.class.getResourceAsStream("custom_chill.ttf");
    ...
    private void initComponents() {
        ...
        setTitle("Ace Combat 3 Text Editor v1.3.4a by Dashman,Modified For Chinese FONT by aikika");
        setResizable(false);
    ...
    public void writeEdit(String text){
        ...
        if (!fontLoaded){
            ...
            if (smallFont)
                ...
                g2d.setFont(new Font("Microsoft YaHei", Font.PLAIN, 10));//这里改小字体
            else{
                ...
                g2d.setFont(ac3font.deriveFont(20f));   //默认字大小 字号
                //g2d.setFont(ac3font.deriveFont(17.5f)); 
                //对话文本：
                //剧情对话框  寒蝉20f
                //履历标题文本：寒蝉 24f
                //百科文本：17.5f
        ...
        //抗锯齿
         //g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON); //关闭 ANTIALIAS_OFF
         //g2d.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_GASP);
         //g2d.setRenderingHint(RenderingHints.KEY_FRACTIONALMETRICS, RenderingHints.VALUE_FRACTIONALMETRICS_ON); 

         g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON); //关闭 ANTIALIAS_OFF
         g2d.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_GASP);
         g2d.setRenderingHint(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_QUALITY); //以提高渲染质量。
         g2d.setRenderingHint(RenderingHints.KEY_STROKE_CONTROL, RenderingHints.VALUE_STROKE_PURE); //确保笔画的精确性。
         g2d.setRenderingHint(RenderingHints.KEY_FRACTIONALMETRICS, RenderingHints.VALUE_FRACTIONALMETRICS_ON);//启用小数像素度量。
        ...
        for (int i = 0; i < lines.length; i++){
            if (!fontLoaded){
                ...
                if (smallFont)
                    ...
                    g2d.drawString(lines[i], 0, i * (fm.getHeight() - 4));
            }
            else{
                if (smallFont){
                    float smallLineHeightSpacing = 3.6f; //默认4 越大越挤
                    float smallTopSpacing = 6f;  //默认6 
                    // Draw the shadow
                    //g2d.setPaint(colour_sub);
                    //g2d.drawString(lines[i], 1, fm.getHeight() - smallTopSpacing + (i*(fm.getHeight()-smallLineHeightSpacing)));

                    // Draw the normal text
                    g2d.setPaint(colour_main);
                    g2d.drawString(lines[i], 0, fm.getHeight() - smallTopSpacing + (i*(fm.getHeight()-smallLineHeightSpacing)));
                }
                else{
                    float lineHeightSpacing = 0.0f;
                    if (checkLineSpacing.isSelected())
                        lineHeightSpacing = 0.2f;//行间距 默认2，，越小越紧凑
                    //对话 行间距  
                    float topSpacing = 4.5f; //顶部间距，越大越靠近顶部边缘 ，
                    //寒蝉CHILL 4.5F 
                    g2d.setPaint(colour_main);  


                    g2d.drawString(lines[i], 0, fm.getHeight() - topSpacing + (i*(fm.getHeight()+ lineHeightSpacing)));   // Antonio-Bold-Custom
                }
            }
        }
